function [targetRawLabel, predictedLabel,mergeSwitchedPosOfRawDataAndRevisied] =  ...
    getRevisedCandidatedPos(testId,cachedData, mapForIdAndName, mapForNameAndId, mapForSegmentation)
%%
% 一个动作的切换点存在以下几种情况：
...
[mergeSwitchedPosOfRawDataAndRevisied, resMergeAllFlag] = mergeWithRevisedPos(mMergeAllPos, revisedMutationPos, 1000);
...
%% 对比实验二
% 不进行平滑，直接使用从原始数据处获得动作切换候选点,平均准确率只有0.8973
% mergeSwitchedPosOfRawDataAndRevisied = targetSegmentPos;

%% 对比实验三
% 只使用平滑后的动作切换点,平均准确率只有0.9270，
% mergeSwitchedPosOfRawDataAndRevisied = revisedMutationPos;
%% 将识别结果、原始标签、切分线综合起来
