function [predict_locs, real_mutation_pos, locs_x, locs_y, locs_z] =  mutationDetectionWithoutLabels(half_win, filtered_x, filtered_y, filtered_z, raw_label, target_id)
...
deviation = 40;
    ...
    figure;
    subplot(311);
    findpeaks(diff_array_x, 'MinPeakHeight',minPeakHeight, 'MinPeakDistance', 100);
    hold on;
    plot(raw_label, 'g-','LineWidth',1.5);
    title('Peaks of filted X');
    subplot(312); 
    findpeaks(diff_array_y, 'MinPeakHeight', minPeakHeight, 'MinPeakDistance', 100);
    hold on;
    plot(raw_label, 'g-','LineWidth',1.5);
    title('Peaks of filted Y');
    subplot(313);
    findpeaks(diff_array_z, 'MinPeakHeight', minPeakHeight, 'MinPeakDistance', 100);
    hold on;
    plot(raw_label, 'g-','LineWidth',1.5);
...
drawPredictedLabel(raw_label, predict_locs, target_id);
    ...
    function [candidate_loc, candidate_count] =  voteForCandidateWithoutLabel(locs, candidate_loc, candidate_count, ~)
        for i = 1 : length(locs)
            detected = 0;
